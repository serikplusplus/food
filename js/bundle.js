...
function calculator() {
  ...
  let sex, height, weight, age, action; //Установка значение по умолчанию с локального хранилища
  ...
  const calcCalories = () => {
    if (sex && height && weight && age && action) {
      if (sex === 'female') {
        calculatingResult.textContent = Math.round((447.6 + 9.2 * weight + 3.1 * height - 4.3 * age) * action);
      } else {
        calculatingResult.textContent = Math.round((88.36 + 13.4 * weight + 4.8 * height - 5.7 * age) * action);
      }
    } else {
  ...
  const setInitialActivity = (selector, activeClass) => {
    ...
    items.forEach(item => {
      ...
      if (item.getAttribute('id') === localStorage.getItem('sex') || item.getAttribute('data-action') === localStorage.getItem('action')) {
...
function sliders({
  ...
  indicators.style.cssText = `
		...
		list-style: none;
	`;
  slider.append(indicators); //Создание точек слайдера

  const dots = [];

  for (let i = 0; i < slides.length; i++) {
    const dot = document.createElement('li');
    dot.setAttribute('data-slide-to', i + 1); //dot.classList.add('dot');
...
const getData = async url => {
  const res = await fetch(url); //Обработка ошибок - fetch не выдает ошибки при отсутствии данных или подключения к базе
...
__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill(); //Полифил для foreach
...
window.addEventListener('DOMContentLoaded', event => {
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_3__.showModal)('.modal', modalTimerId), 20000);
